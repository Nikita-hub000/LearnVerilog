<template>
  <div class="study__header">
    ...
    <button
      class="study__button study__button-edit"
      v-if="!isEdit && isAdmin"
      @click="goEdit"
    >
      Редактировать
    ...
    <input
      class="study__headerText study__input-header"
      v-if="isEdit"
      v-model="renderItem.title"
  ...
  <section>
    <div class="study__lessons">
      <div
        class="study__item"
        :class="{ 'study__item-active': activeItem === item.id }"
      ...
      >
        ...
        ></div>
        <p class="study__itemText padding" @click="goToCourse(item.id)">
          {{ item.titleNav }}
        </p>
      </div>
      <button
        class="study__button study__button-create"
        v-if="isAdmin"
        @click="goToCreate"
      >
        Создать
    ...
    <div class="study__work">
      <p
        class="study__itemText"
        v-if="!isEdit"
        :class="{ 'study__itemText-one': !renderItem.isPractice }"
      >
        {{ renderItem.taskTitle }}
      </p>
      <input
        class="study__itemText"
        v-model="renderItem.taskTitle"
        v-if="isEdit"
        placeholder="Введите название"
      />
      <p
        class="study__itemText"
        :class="{
          'study__itemText-one': !renderItem.isPractice,
          'study__itemText-padding':
            !renderItem.isPractice || renderItem.isTest || renderItem.isOneTest,
        }"
        v-if="
          (!renderItem.isPractice ||
            renderItem.isTest ||
            renderItem.isOneTest) &&
          !isEdit
        "
      >
        {{ renderItem.taskText }}
      ...
      <textarea
        class="study__headerText study__input"
        v-model="renderItem.taskText"
        v-if="isEdit && !renderItem.isPractice"
        placeholder="Введите название"
      ></textarea>
      <img
      ...
      <p
        class="study__hint"
        :class="{ 'study__hint-test': renderItem.isTest }"
        v-if="renderItem.isPractice"
      >
        {{
          renderItem.isTest || renderItem.isOneTest
            ? 'Выберите правильный ответ'
            : 'Введите и отправьте ваше решение в форме ниже'
        }}
      </p>
      ...
      <div v-if="renderItem.isTest">
        <div
          ...
          v-for="(item, index) in renderItem.test"
          :key="index"
        >
          <input
            class="checkbox"
            ...
            :id="'checkbox' + index + 1"
          />
          <label class="label" v-if="!isEdit" :for="'checkbox' + index + 1">{{
            item
          }}</label>
          <label class="label" v-if="isEdit" :for="'checkbox' + index + 1"
            ><input
              v-model="renderItem.test[index]"
              placeholder="Введите значение"
            /><button class="test__delete">&#10006;</button></label
          >
        </div>
      ...
      <textarea
        class="study__headerText study__input"
        v-if="isEdit"
        placeholder="Введите пример кода для задания"
      ></textarea>
      <CodeEditor
        width="800px"
        v-if="
      ...
      <div class="study__sendBox">
        <p class="study__try" v-if="!isEdit && renderItem.isPractice">
          Использовано попыток: 2
        </p>
        <button class="study__button study__button-red" v-if="isEdit">
          Удалить
        </button>
        <div class="box">
          <button class="study__button" @click="success" v-if="!isEdit">
            {{ renderItem.isPractice ? 'Отправить' : 'Продолжить' }}
          </button>
          <p v-show="isCorrect" class="success">Правильно! Вы молодец!</p>
          <p v-show="isInCorrect" class="incorrect">
            Неправильно! <br />
            Попробуйте еще раз:)
          </p>
        </div>
        <button class="study__button" v-if="isEdit" @click="goCourse">
          Сохранить
        </button>
      </div>
    </div>
